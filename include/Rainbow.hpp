...
audio::Sound* rainbow_audio_load_sound(const char* path)
{
    return audio::load_sound(path);
}

audio::Sound* rainbow_audio_load_stream(const char* path)
{
...
FontAtlas* rainbow_fontatlas_create(const char* path, float point_size)
{
...
void rainbow_label_set_text(Label* label, const char* text)
{
    label->set_text(text);
}
...
void rainbow_renderqueue_disable_tag(void* handle, const char* tag)
{
    auto& queue = *static_cast<graphics::RenderQueue*>(handle);
...
void rainbow_renderqueue_enable_tag(void* handle, const char* tag)
{
    auto& queue = *static_cast<graphics::RenderQueue*>(handle);
...
void rainbow_renderqueue_remove(void* handle, const char* tag)
{
    auto& queue = *static_cast<graphics::RenderQueue*>(handle);
    remove(queue, tag);
}

void rainbow_renderqueue_set_tag(void* handle, int32_t unit, const char* tag)
{
    auto& queue = *static_cast<graphics::RenderQueue*>(handle);
...
TextureAtlas* rainbow_textureatlas_create(const char* path, float scale)
{
